...
class PolicyNet(nn.Module):
    def __init__(self, action_dim):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(32, 32, kernel_size=3, padding=1, stride=2),  # 210 - 3 + 2 x 1 / 2 + 1 = 105, 160 - 3 + 2 x 1 / 2 + 1 = 80
            nn.BatchNorm2d(32),
            nn.Mish(),
            nn.Conv2d(32, 64, kernel_size=3, padding=1, stride=3),  # 105 - 3 + 2 x 1 / 3 + 1 = 35, 80 - 3 + 2 x 1 / 3 + 1 = 27
            nn.BatchNorm2d(64),
            nn.Mish()
        )

        self.conv3 = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=3, padding=1),    # 35 - 3 + 2 x 1 / 1 + 1 = 35, 27 - 3 + 2 x 1 / 1 + 1 = 27
            nn.BatchNorm2d(64),
            nn.Mish(),
            nn.Conv2d(64, 128, kernel_size=3, padding=1),   # 35 - 3 + 2 x 1 / 1 + 1 = 35, 27 - 3 + 2 x 1 / 1 + 1 = 27
            nn.BatchNorm2d(128),
            nn.Mish()
        )

        self.linear = nn.Sequential(
            nn.Linear(128 * 35 * 27, 256),
            nn.LayerNorm(256),
            nn.Mish(),
            nn.Linear(256, 32),
            nn.LayerNorm(32),
...
class QValueNet(torch.nn.Module):
    def __init__(self, action_dim):
        ...
        # 输入尺寸为210x160x3
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 16, kernel_size=3, padding=1),     # 210 - 3 + 2 x 1 / 1 + 1 = 210
            nn.BatchNorm2d(16),
            nn.Mish(),
            nn.Conv2d(16, 32, kernel_size=3, padding=1),    # 210 - 3 + 2 x 1 / 1 + 1 = 210
            nn.BatchNorm2d(32),
            nn.Mish(),
        )

        self.conv2 = nn.Sequential(
            nn.Conv2d(32, 32, kernel_size=3, padding=1, stride=2),  # 210 - 3 + 2 x 1 / 2 + 1 = 105
            nn.BatchNorm2d(32),
            nn.Mish(),
            nn.Conv2d(32, 64, kernel_size=3, padding=1, stride=3),  # 105 - 3 + 2 x 1 / 3 + 1 = 35
            nn.BatchNorm2d(64),
            nn.Mish()
        )

        self.conv3 = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=3, padding=1),    # 35 - 3 + 2 x 1 / 1 + 1 = 35
            nn.BatchNorm2d(64),
            nn.Mish(),
            nn.Conv2d(64, 128, kernel_size=3, padding=1),   # 35 - 3 + 2 x 1 / 1 + 1 = 35
            nn.BatchNorm2d(128),
            nn.Mish()
        )
