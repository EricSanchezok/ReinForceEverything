...
def train_off_policy_agent(env_name, replay_buffer, agent, difficulty, num_episodes, max_step_per_epoch, minimal_size, batch_size, device):
    ...
    # 检查有没有之前训练的模型，有的话就加载
    try:
        agent.actor.load_state_dict(torch.load('Boxing/agent/actor_v1.pth', map_location=torch.device(device)))
        agent.critic.load_state_dict(torch.load('Boxing/agent/critic_v1.pth', map_location=torch.device(device)))

        agent.target_actor.load_state_dict(torch.load('Boxing/agent/actor_v1.pth', map_location=torch.device(device)))
        agent.target_critic.load_state_dict(torch.load('Boxing/gent/critic_v1.pth', map_location=torch.device(device)))
    
        print('Model loaded!')
    except:
        torch.save(agent.actor.state_dict(), 'Boxing/agent/actor_v1.pth')
        torch.save(agent.critic.state_dict(), 'Boxing/agent/critic_v1.pth')
